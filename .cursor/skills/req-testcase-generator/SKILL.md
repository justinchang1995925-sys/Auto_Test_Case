...
- 需统计：反模式命中/已修复数、追溯缺口数（含 REQ→TP、TP→TC、REQ→TC）、TP 无 REQ 关联数、**一对多坍缩数（含「多 TP 共用一条 TC」与「多 REQ 共用一条 TC」两类，见硬门禁第 11 项①②③）**、**TP 无专属 TC 数**、覆盖深度缺口数、待人工确认数
...
12. **交付件排版检查（强制，对实际生成的每一套——默认飞书，若另出本地则本地也查）**：
