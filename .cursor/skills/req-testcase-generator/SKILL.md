...
1. **一件一文件**：每个交付件独立，主表与附表分开、追溯表单独成表，不得合并列拼在一张表。
2. **Excel 交付件（强制）**：除思维导图外，所有交付件的最终交付形式**必须是 Excel（飞书电子表格或本地 `.xlsx`），严禁以 `.md`、`.csv`、`.txt` 或对话内 Markdown 表格作为正式交付**。Markdown 表格仅用于对话中预览，确认后必须落成 Excel。
   - 优先导入飞书电子表格：切 [`lark-sheets`](../../../.agents/skills/lark-sheets/SKILL.md)，每个交付件建独立 Sheet，Sheet 名用上表中文名称。
   - 用户要本地文件：用 `openpyxl` 生成 `.xlsx`，每个交付件一个独立 Sheet（或每件一个独立 `.xlsx`），文件名 `测试交付件-{需求名}-{日期}.xlsx`。不得用 `.md`/`.csv` 顶替。
...
5. **必须产出真正的 `.xlsx` 文件（强制）**：除思维导图外的交付件，最终必须是**真正的 `.xlsx` 二进制文件**，使用者下载后可直接用 Excel / 飞书表格/WPS **打开、编辑、保存**。
   - 本地交付：用 `openpyxl`（或 `pandas` + `openpyxl` 引擎）真实写盘生成 `.xlsx`，**不得**把 Markdown/CSV 文本改个后缀冒充，也不得只贴表格文本让用户自行转换；生成后**告知文件的实际保存路径**。
   - 飞书交付：切 `lark-sheets` 建真实电子表格并返回可访问链接；用户若要 `.xlsx`，用 `lark-drive` 导出为 `.xlsx` 后提供。
   - 交付前自检：文件确为 `.xlsx`（可被表格软件打开、非纯文本）、可编辑保存、各交付件独立、表头加粗+筛选+列宽自适应、名称为中文。
...
- 正式交付按「交付物规范」**必须落成 Excel**：**每个交付件独立成表/独立文件**，不合并，不得用 `.md`/`.csv` 顶替。
- 用例 ≤ 30 条：可在对话中输出各表完整预览后再落成 Excel。
- 用例 > 30 条：主表按模块分 Sheet，但主表/附表/追溯表/专项表仍各自独立，不因分模块而混表。
- 落表优先切 [`lark-sheets`](../../../.agents/skills/lark-sheets/SKILL.md) 建多 Sheet 工作簿；用户要本地文件则用 `openpyxl` 生成 `.xlsx`（多 Sheet 或每件独立 `.xlsx`）。
