...
| 测试点思维导图 | 六维度测试点，命名 `XX项目测试点` | Phase 2 | Draw.io（`.drawio`，可导入飞书云文档） |
...
6. **思维导图交付件（Draw.io，完全适配飞书在线思维导图）**：
   ...
   - 层级与 Mermaid 一致：根=需求名 → 维度 → 测试点，节点文本以 `TP-ID` 开头，可追溯回追溯表。模板见 [templates.md](templates.md) 「思维导图 Draw.io 格式」。
7. **测试计划交付件（本地 Markdown + 飞书在线文档，双份强制）**：
   - **默认同时交付两套**：本地一份 `.md` + 飞书一份在线文档（切 [`lark-doc`](../../../.agents/skills/lark-doc/SKILL.md) 用 `docs +import`/创建导入为在线文档），交付时同时给出本地路径与飞书文档链接（用户明确只要一种时除外）。
...
对话中用 Mermaid 预览；**正式交付时另出一份 Draw.io 文件（命名 `XX项目测试点.drawio`），可直接导入飞书云文档**，导入规则见「交付物规范」。层级与节点文本须与本 Mermaid 一致（根=需求名 → 维度 → 测试点，节点带 `TP-ID`）。
...
- **覆盖率、可执行性、质量缺陷类型、优先级分布、硬门禁均用饼图并显示占比 xx%**（覆盖率=已覆盖/未覆盖/N-A；可执行性=Ready/Blocked/Draft；缺陷=各缺陷类型占比；优先级=P0/P1/P2/P3，叠加建议比例对照；硬门禁=各门禁项通过/未通过占比）——全部为饼图，飞书与本地两套一致。
...
- 需统计：反模式命中/已修复数、追溯缺口数（含 REQ→TP、TP→TC、REQ→TC）、TP 无 REQ 关联数、**一对多坍缩数（多 TP 共用一条 TC）**、**TP 无专属 TC 数**、覆盖深度缺口数、待人工确认数
...
11. **一对多坍缩扫描（强制）**：检查是否存在多个 `TP-ID` 共用同一条 `TC-ID`、或一条 TC 同时挂多个 REQ/TP 的现象。逐条核对追溯矩阵，若某 `TP-ID` 没有专属 TC，或某一行/单元格塞入多个 TP/TC，即判为坍缩缺陷，必须拆分为独立用例后复审
12. **交付件排版检查（强制，本地 Excel 与飞书在线表格两套都要检查）**：
...
- 优先级缺依据数 = 0
- 优先级定级不合理数 = 0
- 一对多坍缩数 = 0（无多 TP 共用一条 TC）
- TP 无专属 TC 数 = 0
- 交付件排版问题数 = 0（需求清单来源、审计报告说明均在顶栏合并单元格；测试计划含修订记录）
