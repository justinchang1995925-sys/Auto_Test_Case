...
| 交付件 | 内容 | 对应阶段 | 形式 | 建议文件/Sheet 名 |
|--------|------|---------|------|------------------|
| 需求原子化清单 | REQ 条目表 | Phase 1.5 | Excel | `需求清单` |
| 测试点思维导图 | 六维度测试点 | Phase 2 | 飞书思维导图（缩进大纲，可导入） | `测试点思维导图.{md,opml}` |
| 测试用例（主表） | 执行用例，固定表头 | Phase 3 | Excel | `测试用例` |
| 测试用例附表 | 评审追溯、风险、用例状态 | Phase 3 | Excel | `用例附表` |
| 追溯表 | `REQ → TP → TC` 追溯矩阵 | Phase 3 | Excel | `追溯表` |
| 专项测试用例 | 专项指标定义表（如有专项测试） | Phase 3 | Excel | `专项测试用例` |
| 专项数据采集表 | 每条专项用例 N 次采样统计（如有专项测试） | Phase 3 | Excel | `专项数据采集` |
| 质量审计报告 | 覆盖率/可执行性/质量缺陷三报告 | Phase 4 | Excel | `质量审计` |
...
2. **Excel 交付件（强制）**：除思维导图外，所有交付件的最终交付形式**必须是 Excel（飞书电子表格或本地 `.xlsx`），严禁以 `.md`、`.csv`、`.txt` 或对话内 Markdown 表格作为正式交付**。Markdown 表格仅用于对话中预览，确认后必须落成 Excel。
   ...
   - 用户要本地文件：用 `openpyxl` 生成 `.xlsx`，每个交付件一个独立 Sheet（或每件一个独立 `.xlsx`），文件名 `testcase-deliverables-{需求名}-{日期}.xlsx`。不得用 `.md`/`.csv` 顶替。
3. **思维导图交付件（可导入飞书在线思维导图）**：
...
4. 阶段性说明、建模产物（判定表/状态图等）在对话中呈现，不单独作为交付件，但其信息须能追溯到上述表。
