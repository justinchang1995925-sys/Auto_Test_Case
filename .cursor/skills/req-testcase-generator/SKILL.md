...
所有正式交付件必须**独立分开呈现，一个交付件一个文件，禁止把多种交付件混在一起**。**除测试点思维导图外，其余交付件一律用 Excel 形式**；思维导图单独交付为**可直接导入飞书在线思维导图（思维笔记）的格式**。最终交付清单如下：
...
| 测试点思维导图 | 六维度测试点 | Phase 2 | 飞书思维导图（缩进大纲，可导入） |
...
1. **一件一 Sheet，汇入同一个工作簿（强制）**：除思维导图外，所有 Excel 交付件组织为**同一个 Excel 工作簿内的多个独立 Sheet**（一个交付件 = 一个 Sheet），便于统一管理。主表与附表分开、追溯表单独成 Sheet，**不得把多种交付件合并列拼在同一张 Sheet 里**。Sheet 顺序建议按上表交付清单顺序排列。
2. **Excel 交付件（强制）**：除思维导图外，所有交付件的最终交付形式**必须是 Excel（飞书电子表格或本地 `.xlsx`），严禁以 `.md`、`.csv`、`.txt` 或对话内 Markdown 表格作为正式交付**。Markdown 表格仅用于对话中预览，确认后必须落成 Excel。
   - 统一为**一个工作簿多 Sheet**：本地 `openpyxl` 生成单一 `.xlsx`（多 Sheet），文件名 `测试交付件-{需求名}-{日期}.xlsx`；飞书切 [`lark-sheets`](../../../.agents/skills/lark-sheets/SKILL.md) 建**一个电子表格**、每个交付件一个 Sheet。Sheet 名用上表中文名称。不得用 `.md`/`.csv` 顶替。
...
5. **默认双份交付：本地 `.xlsx` + 飞书在线电子表格（强制）**：除思维导图外的交付件，**默认同时生成两套**，两套内容一致（同样的多 Sheet 结构、格式与中文命名），除非用户明确只要其中一种：
   - **本地一份**：用 `openpyxl`（或 `pandas` + `openpyxl` 引擎）真实写盘生成**一个多 Sheet 的 `.xlsx` 二进制文件**，使用者可用 Excel/WPS 打开、编辑、保存；**不得**把 Markdown/CSV 改后缀冒充或只贴文本让用户自行转换；生成后**告知文件的实际保存路径**。
   - **飞书一份**：切 [`lark-sheets`](../../../.agents/skills/lark-sheets/SKILL.md) 建**一个在线电子表格**（每个交付件一个 Sheet），并返回可访问链接。
   - 两套建议同名（本地文件名与飞书表格标题一致），交付时同时给出**本地路径**与**飞书链接**。
   - 交付前自检：本地文件确为 `.xlsx`（可被表格软件打开、非纯文本）且可编辑保存、飞书表格链接可访问；两套均满足各交付件独立成 Sheet、表头加粗+筛选+列宽自适应、名称为中文。
6. **思维导图交付件（可导入飞书在线思维导图）**：
   - 对话中用 Mermaid `mindmap` 预览；正式交付**另出一份可直接导入飞书思维笔记的文件**。
   - 首选缩进大纲格式：**Markdown 大纲**（`#`/缩进 `-` 层级，根为需求名，逐级为维度→测试点，节点文本带 `TP-ID`）或 **OPML**。飞书思维笔记「导入」支持这类大纲，导入后即成在线思维导图。
   - 若用户要直接落到飞书：切 [`lark-doc`](../../../.agents/skills/lark-doc/SKILL.md) 思维笔记链路——向**已有**思维笔记写节点用 `lark-cli mindnotes nodes create`（逐层建 `parent_id`）；**新建**思维笔记走 `lark-doc` 画板/思维笔记新建链路。先向用户确认是新建还是写入已有笔记。
   - 无论哪种形式，思维导图节点必须能追溯到 `TP-ID` 与对应 `REQ-ID`（与追溯表一致）。
7. 阶段性说明、建模产物（判定表/状态图等）在对话中呈现，不单独作为交付件，但其信息须能追溯到上述表。
8. 交付前核对：上表中「有专项测试才输出」的两项按实际情况取舍，其余为必交项。
...
对话中用 Mermaid 预览；**正式交付时另出一份可直接导入飞书在线思维导图的缩进大纲文件（Markdown 大纲 / OPML）**，导入规则见「交付物规范」。层级与节点文本须与本 Mermaid 一致（根=需求名 → 维度 → 测试点，节点带 `TP-ID`）。
...
9. **覆盖深度**：每个可测功能 REQ 至少包含正向 + 反向/异常用例（见 Phase 2 最低标准）
10. **用例状态**：附表必须为每条用例标注 `Ready` / `Blocked` / `Draft`；`Blocked` 必须写解除条件
...
其中「设计技法」列需在附表 Sheet 附「设计技法说明」（各技法含义），「优先级评估」列须逐条解释该用例定为 P0/P1/P2/P3 的理由（判定基准见 [templates.md](templates.md)「优先级评估规范」）。
...
- **默认同时交付两套**：本地一个多 Sheet `.xlsx` + 飞书一个在线电子表格，同时给出本地路径与飞书链接（用户明确只要一种时除外）。
...
**Sheet 组织要求（强制）**：
- Sheet 顶部放一段**总说明**，解释覆盖率报告 / 可执行性报告 / 质量缺陷报告 / 优先级合理性评估 / 硬门禁各自做什么。
- **每个检查项既有表格数据、又配可视化图表**（飞书用 `lark-sheets` 图表、本地用 `openpyxl.chart`），如覆盖率柱状、Ready/Blocked/Draft 饼图、缺陷类型柱状、优先级分布柱状。
