...
| 测试点思维导图 | 六维度测试点 | Phase 2 | Draw.io（`.drawio`，可导入飞书云文档） |
| 测试计划 | 范围目标/策略/资源/进度/准入准出/风险/缺陷流程/交付物清单 | Phase 3 | Markdown（`.md`，可导入飞书在线文档） |
...
5. **默认双份交付：本地 `.xlsx` + 飞书在线电子表格（强制）**：表格类交付件（除思维导图、测试计划外），**默认同时生成两套**，两套内容一致（同样的多 Sheet 结构、格式与中文命名），除非用户明确只要其中一种：
...
6. **思维导图交付件（Draw.io，可导入飞书云文档）**：
   - 对话中用 Mermaid `mindmap` 预览；正式交付**另出一份 Draw.io 文件（`.drawio`，即 mxGraphModel XML）**，命名 `测试点思维导图.drawio`。
   - 层级与 Mermaid 一致：根=需求名 → 维度 → 测试点，节点文本以 `TP-ID` 开头，保证可追溯回追溯表。
   - 导入方式：飞书云文档 / 画板支持导入 Draw.io；或切 [`lark-doc`](../../../.agents/skills/lark-doc/SKILL.md) 画板链路落到飞书。模板见 [templates.md](templates.md) 「思维导图 Draw.io 格式」。
7. **测试计划交付件（Markdown，可导入飞书在线文档）**：
   - 单独交付一份 `测试计划.md`，**必须包含**：测试范围与目标、测试策略与方案概述、资源安排、进度计划、准入标准、准出标准、风险分析与应对、缺陷管理流程、交付物清单。
   - **测试策略须含按测试类型的优先级排序**：功能/接口测试先通过 → 再性能/稳定性测试 → 最后专项测试，前一阶段有阻塞级缺陷不进入下一阶段，并配 Mermaid 流程图。
   - **准出标准须为**：用例执行率 100%、无阻塞级缺陷、遗留缺陷均经过评审且有处理结论或应对方案（另含覆盖/深度达标、专项指标达标如有）。
   - **缺陷管理流程**一节可留白（占位，后续手动贴链接/附件）；其余各节须结合本需求写实质内容。
   - 交付为 `.md`（飞书在线文档「导入」支持 Markdown），需要时切 `lark-doc` 导入为在线文档。完整模板见 [templates.md](templates.md) 「测试计划模板」。
8. 阶段性说明、建模产物（判定表/状态图等）在对话中呈现，不单独作为交付件，但其信息须能追溯到上述表。
9. 交付前核对：上表中「有专项测试才输出」的两项按实际情况取舍，其余为必交项。
...
对话中用 Mermaid 预览；**正式交付时另出一份 Draw.io 文件（`测试点思维导图.drawio`），可直接导入飞书云文档**，导入规则见「交付物规范」。层级与节点文本须与本 Mermaid 一致（根=需求名 → 维度 → 测试点，节点带 `TP-ID`）。
...
- **每个检查项既有表格数据、又配可视化图表，且飞书 Sheet 与本地 Excel 两套都要生成图表**（飞书用 `lark-sheets` 图表、本地用 `openpyxl.chart`）。
- **可执行性用饼图并显示占比 xx%**（Ready/Blocked/Draft）；**优先级分布用饼图并显示占比 xx%**（P0/P1/P2/P3）；覆盖率用柱状+深度占比饼图；缺陷类型用柱状。
...
| 「生成测试计划」 | 输出 `测试计划.md`（范围/策略/资源/进度/准入准出/风险/缺陷流程留白/交付物清单） |
| 「导出思维导图」「出 Draw.io」 | 输出 `测试点思维导图.drawio`，可导入飞书云文档 |
