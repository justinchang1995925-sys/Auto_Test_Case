...
| Phase 2 | 已确认 REQ 范围 | 六维度拆解测试点 | 思维导图 + 测试点汇总 |
| Phase 3 | 已确认测试点 | 生成执行主表、追溯附表和追溯矩阵 | 主表 + 附表 + 追溯矩阵 |
| Phase 4 | 主表 + 附表 + 追溯矩阵 | 生成三报告并执行硬门禁 | 最终可发布版本或阻断清单 |
...
约束：
- 每个 `可测` 的 `REQ-ID` 至少映射 1 条 `TC-ID`
- 每条 `TC-ID` 必须能回溯到 `REQ-ID`
- 未映射项必须进入 Phase 4 覆盖率报告
