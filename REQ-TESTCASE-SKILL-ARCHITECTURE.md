...
所有正式交付件**独立分开**呈现，不得混在一起：**表格类交付件**（需求清单、测试用例、用例附表、追溯表、专项测试用例、专项数据采集、质量审计）一律以 **Excel** 独立成表；**测试点思维导图**单独交付为 **Draw.io（`.drawio`）文件**，可直接导入飞书云文档；**测试计划**单独交付为 **Markdown（`.md`）文档**，可导入飞书在线文档。详见 `SKILL.md` 「交付物规范」。
...
| 质量审计组件 | 检查反模式、覆盖率、可执行性、追溯完整性、优先级合理性 | 四报告（含总说明与图表）与阻断项 |
...
| Phase 2 | 已确认 REQ 范围 | 六维度拆解 + 强制建模 + 深度覆盖 | 思维导图（Draw.io）+ 测试点汇总（含 REQ） |
| Phase 3 | 已确认测试点 | 生成主表、附表（含状态）、追溯矩阵、测试计划 | 主表 + 附表 + 追溯矩阵 + 测试计划（.md） |
| Phase 4 | 主表 + 附表 + 追溯矩阵 | 四报告（含深度）+ 硬门禁 | 最终可发布版本或阻断清单 |
...
- 步骤可复现
- 预期可观测、可判定
- 操作步骤与预期结果逐条一一对应、条数相等（严禁多步骤对一条预期）
...
- 测试数据需具体值，不可抽象描述
- **设计技法列须在该 Sheet 附「设计技法说明」**（解释等价类/边界值/判定表/状态迁移/场景法/错误推测的含义）
- **优先级评估列必填**：逐条解释该用例为何定为 P0/P1/P2/P3，须与风险等级一致
...
交付形式：**默认只交付飞书在线文档一份**（由 Markdown 导入/创建为在线文档，返回文档链接）；**仅当用户明确要求本地文件时才额外生成本地 `.md`** 并给出本地路径。**命名 `XX项目测试计划`**（如 `咖啡机项目饮品APP测试计划`）。
...
约束：
- **文档最上方必须有「修订记录」表**（版本号 / 修订日期 / 修订人 / 审核人 / 修订内容 / 备注），每次修订追加一行，不得删改历史行
- **测试策略与方案概述用表格呈现**，并按测试类型排优先级：功能/接口测试先通过 → 性能/稳定性测试 → 专项测试 → 回归测试；前一阶段有阻塞级缺陷不进入下一阶段
...
- **流程图用第三方画图 skill（如 archify）生成的图片/图形直接嵌入文档**（导出 PNG/SVG），替代 Markdown 版 Mermaid 源码块（Mermaid 仅在对话预览用）
- 完整模板见 `templates.md`「测试计划模板」；规则详见 `SKILL.md`「交付物规范」
...
为了支持“需求覆盖 100%”与“可执行度 100%”的判断，Skill 在 Phase 4 的「质量审计报告」Sheet 强制输出四份报告（含顶部总说明，每项既有表格数据又有可视化图表）：

1. 覆盖率报告（含覆盖深度）
2. 可执行性报告（基于附表用例状态列）
3. 质量缺陷报告
4. 优先级合理性评估（核对定级是否合理、分布是否健康，直接影响测试策略实施质量）
