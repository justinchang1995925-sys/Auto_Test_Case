...
**「导入/导出/复制」类需求必须问清副本语义**。导入后是独立副本还是引用同一份，
决定了要不要设计「多实例隔离性」用例。实测漏过：地图导入多台机器复用，隔离性无人验证——
若实为共享同一份，一台上误删会波及全线，而这个风险在需求文档里一个字都没提。
故 Phase 1.2 的 C 组把它列为固定提问。

**六维度检查清单不强制过，等于没有**。清单里本来就写着「断网/弱网恢复」
「并发操作（重复提交）」「重试与幂等」，实测仍全部漏掉。原因是设计路径绕过了它：
按「读需求/配置里写明的判定 → 写正向 + 反向」推，反向面只覆盖**文档记载的错误码**
（`CAMERA_CMD_FAILED`、`META_NOT_READY` 这类），而交互与环境类异常**文档不会写**。
实测代价：某项目权限开关漏了断网与连续点击（界面已开、实际未开的偏差无人验），
归档漏了传输中断网（半份文件被当成有效归档），都是评审时业务方指出才补。
故 Phase 2 要求逐项回报「已覆盖(哪个TP) / 不适用(+具体理由)」——
判为不适用要写机制层面理由，与 EX 矩阵同源。

**「疑似重复用例」只作软提示**（`dup_cand`，判据=步骤与预期逐条相同，不比标题）。
为什么必然复发：深度门禁要求某 REQ 补某方向覆盖时，**最省事的做法就是复制邻近用例
改个 REQ 号交差**。实测踩过：为给 REQ-012（讲「不符时报错」）补正向面，
复制了 REQ-010 的「版本一致判 PASS」——而那本就是 REQ-010 的地盘，
REQ-012 的正向面应是「报错内容本身完整正确」。改写后两条各守其事。
不作阻断的理由：存在「步骤相同但测试数据不同」的合规写法（同一操作换参数）。
