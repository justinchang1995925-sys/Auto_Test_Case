...
**A 组：脚本自动判定（13 项，`tcgen.audit.gates()`）**
...
| B9 | 交付件全链路同步 = 0 差异 | `sync_feishu.py diff` 须 0 差异 + 图表引用对齐（见门禁 18） |
