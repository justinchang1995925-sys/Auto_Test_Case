...
- **用例维度以其「测试类型」判定，再归并到六维度**：`功能测试 / 接口测试 / 反向测试 / 异常测试 / 边界测试` 都属于**功能维度**（反向/异常/边界/接口是功能维度内的**技法差异**，不是独立维度，挂在 `TP-F-xxx` 下完全正确）；`性能测试→性能`、`稳定性测试→稳定性`、`兼容性测试→兼容性`、`安全测试→安全`、`用户体验测试→用户体验`。
...
6. **测试类型**：功能测试 / 接口测试 / 性能测试 / 兼容性测试 / 安全测试 / 稳定性测试 / 用户体验测试 / 专项测试
...
- 未覆盖 REQ 数 = 0
- 深度未达标 REQ 数 = 0
- 功能安全深度未达标 REQ 数 = 0（每个功能安全 REQ 同时具备「触发」与「解除/恢复」覆盖，多触发源每源 ≥1 条）
- REQ 清单结构性完整（来源章节反查差集 = 0、编号无断号/重号、必填字段齐全、分母口径自洽）
- Blocked 用例数 = 0
- Draft 用例数 = 0
- 反模式命中数已清零
- 追溯缺口数 = 0
...
- 优先级缺依据数 = 0
- 优先级定级不合理数 = 0
- 一对多坍缩数 = 0（既无多 TP 共用一条 TC，也无多 REQ 共用一条 TC，且矩阵归属与 TC 字段一致）
- TP 无专属 TC 数 = 0
- TP 维度与用例维度错配数 = 0（每条 TC 的测试类型所属维度与其 TP 前缀维度一致）
- **标题写验证方法数 = 0**（标题须写「条件+行为+预期」，不得写成「X 由 Y 验证」这类验证手段说明）
- **覆盖类型与测试类型矛盾数 = 0**（反向测试↔反向、边界测试↔边界、异常测试↔异常；性能/稳定性/兼容性测试不受此约束，其覆盖类型按后果归属定，见 [exception-library.md](exception-library.md)）
- 技法与测试类型不相容数：**软提示**，只列待人工确认项，不阻断（边界情形确实存在，如状态迁移技法配边界测试）
- EX 无本体用例数 = 0、交叉矩阵未评估格数 = 0、矩阵判定缺依据数 = 0、必测交叉缺口 = 0、EX-ID 引用非法数 = 0（仅当本期纳入 EX 时校验；标 `不适用` 且依据充分的格不计入缺口）
- 交付件排版问题数 = 0（需求清单来源、审计报告说明均在顶栏合并单元格；测试计划含修订记录；思维导图出到用例层、≥12 TP 的维度有中文模块层、同层零重叠、父子距离 ≤600px、无裸用例节点、模块顺序与 TP 编号一致，且画板已回读复验）
- 每个门禁数均附计算口径（自查回执），且 >0 项均已列出违例 ID（审计三铁律）
