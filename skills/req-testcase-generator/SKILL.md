...
| ② 前端实现 | 读前端 bundle 的控件、参数、枚举、禁用条件、状态分支 | 组合边界、非法组合防护、控件联动逻辑只能从这里看出来；详见下方「前端实现摄入规范」 |
| ③ 只读接口实测 | 调只读接口看真实返回 | 拿到真实数据形态与当前环境状态 |
| ④ 版本核对 | 确认系统当前版本，并记录进用例备注 | 实现形态跟版本绑定，换版本要能快速复查 |

#### 前端实现摄入规范（步骤 ② 展开，强制）

前端实现是需求文档之外最重要的用例输入源。文档只写"支持三种类型"，前端才能告诉你三种类型的枚举值是什么、选不同值后界面怎么变、哪个控件在什么条件下被锁定。以下四项**全部要摄入**，缺项不得进入 Phase 2。

**一、必须摄入的四类信息（颗粒度要求）**

| 类别 | 摄入什么 | 为什么不能少 |
|------|---------|------------|
| **枚举值与控件选项** | 下拉框/单选组/切换器的完整选项列表，含前端内部 key 值（如 `gripper_v1` / `none` / `custom`）和展示文案（如「标配夹爪」） | 决定等价类划分与组合遍历的**实际分母**；光靠需求文档常拿不到 key 值，用例操作步骤里的输入数据必须用 key 值（或与 key 值对应的文案），不能只写"选一种类型" |
| **控件联动与条件显示** | 选项 A 时哪些字段出现/隐藏；选项 B 时哪些字段置灰/变必填；是否左右臂共用一个设置还是各自独立 | 联动规则决定**组合空间的形状**（不是笛卡尔积）和边界用例；联动用文档几乎不写，漏了就漏了整类测试点 |
| **禁用与锁定条件** | 控件/区块何时 `disabled`/带 `locked` 类；锁定由哪个状态变量（如 `ne.value`）控制；锁定时点击是否有反馈 | 锁定状态是反向和状态迁移用例的核心检查点；若只知道"不可点"但不知道触发条件，就写不出"什么情况下解锁"的完整预期 |
| **状态分支与中止路径** | 同一操作在不同状态下走哪条分支（如"中止"≠"未通过"，是两个独立分支）；各分支的 UI 文案、状态变量终态 | 分支数决定需要多少条状态迁移用例；若只看需求文档，常把"中止"与"失败"误读为同一结果，漏掉一条分支就漏掉一批用例 |

**二、从摄入结论到用例设计的联动规则（强制）**

摄入前端实现后，必须按以下联动规则驱动用例设计，不得只把结论写进备注而不影响用例：

1. **枚举值 → 等价类划分**：每个枚举值是一个等价类，等价类数量决定正向用例的最低条数。三选项就要三条（或合理合并后说明依据），不能只写"选一种验证通过"。
2. **联动字段 → 边界用例**：条件显示的字段（如 `custom` 才出现的配重行），必须对应一条"该字段出现/不出现"的功能用例，以及一条"仅填一侧/全空/全填"的边界用例；缺字段出现验证等于用例前提条件不可核对。
3. **锁定条件 → 状态迁移用例对**：每个锁定状态必须同时设计"锁定中不可操作"（反向）和"解锁后可操作"（正向）两条用例，构成一对；只写锁定、不写解锁，覆盖深度不达标。
4. **状态分支 → 独立用例**：不同分支（如"中止"与"未通过"）必须各自有独立用例；严禁把两个分支合并进一条用例的预期，否则任一分支走错都无法定位。

**三、界面实测结论的记录格式（强制）**

界面实测发现的内容必须**同时记录在两处**，缺一不可：

- **用例 `remark` 字段**：写 `界面实测:<发现的事实>`，格式固定，便于后续审计与版本更新时快速定位"这条用例依赖哪个前端实现细节"。
  - 正确示例：`界面实测:3选项,左右臂共用一个设置`
  - 正确示例：`界面实测:custom 才显示配重行,两侧都必填`
  - 正确示例：`界面实测:dc-confirm 带 locked 类,ne.value 控制解锁`
  - 正确示例：`界面实测:中止与未通过是两条分支`
  - **禁止**只写发现结论而不加 `界面实测:` 前缀（会与需求文档来源的备注混淆）
  - **禁止**把内部变量名/CSS 类名只写进代码注释而不写进用例 remark（换人接手时找不到依据）

- **用例操作步骤与预期**：实测结论必须**落入具体步骤和可观测预期**，不能停在 remark 里当"参考信息"。
  - 枚举值发现 → 操作步骤写具体选项文案（`选「标配夹爪（gripper_v1）」`），预期写对应行为
  - 联动字段发现 → 操作步骤写"选 custom 后观察是否出现配重行"，预期写"出现左臂 kg / 右臂 kg 两个输入框"
  - 锁定条件发现 → 预期写"整块为锁定态，三个复选框均不可勾选，点击无效"
  - 状态分支发现 → 预期写区分两个分支的可观测差异（如"面板提示『标定已中止』不报『未通过』"）

**四、摄入完成的验收标准（摄入步骤 ② 的放行条件）**

以下三项全部满足才算步骤 ② 完成，否则不得推进到 Phase 1：

- [ ] 每个含多选项控件的需求，已列出完整枚举值（含 key 值）并标注左右臂是否共用
- [ ] 每个条件显示/隐藏字段，已记录触发条件（选哪个选项才出现、哪些子字段是必填）
- [ ] 每个被锁定的控件/区块，已记录锁定的触发变量或 CSS 类名，以及解锁条件
...
- **C. 业务规则待明确（文档提到但没讲清）**：文档已提及但逻辑不完整、缺分支、缺异常处理的规则。**需求含「导入/导出/同步/复制/拷贝」时，本组必须固定问一条副本语义**：导入后是独立副本还是引用同一份？多实例导入同一份数据时，一处改动会否波及其他？答案决定要不要设计「多实例隔离性」用例。
...
**六维度检查清单必须逐项过一遍并回报（强制）**：
[templates.md](templates.md)「六维度检查清单」每项须给出「已覆盖(哪个 TP)」或
「不适用(+具体理由)」，Phase 2 结束时一并回报；不接受「不涉及」这类无依据判断。
**交互与环境类异常（断网、重复点击、传输中断）是文档不会记载的那部分，
不主动过清单就想不到**——漏检案例见 [pipeline.md](pipeline.md)。
...
13. **通用异常交叉用例（EX，如急停/低电量/定位丢失）**：规则与模板全在
    [exception-library.md](exception-library.md)，**动手前必读**。四条不可违反的要点：
    - **必须先有矩阵**：只按「异常交叉适用性矩阵」标 `必测`/`选测` 的格写，严禁凭感觉挑场景硬加；
      标 `不适用` 且依据充分的格不补用例、不计缺口。
    - **不能交互的不写**：判 `必测/选测` 须同时满足「时机可达 + 机制有耦合 + 有独立可观测差异」。
    - **REQ 主归属异常侧**：挂 EX 实例化的异常 REQ，业务场景写备注列（前缀 `交叉基线：`）；
      严禁把业务 REQ 填进矩阵 `REQ-ID` 列（会触发门禁 11②）。
    - **维度按后果判**：伤人/损机 → 安全（`TP-SEC-101~199`）；任务失败/数据不一致 → 稳定性
      （`TP-S-xxx`）。单格可升级，须在矩阵写明原因。不新增测试类型、不新增第七维度。
